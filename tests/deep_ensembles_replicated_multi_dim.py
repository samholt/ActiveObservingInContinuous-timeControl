...
def plot_one_mse_model(gt_X, gt_y, X, y, config):
    ...
    # One MSE model
    model = MLP(config.dims, config.dims)
...
def plot_one_gaussian_model(gt_X, gt_y, X, y, config):
    seed_all(config.seed)
    # One NLL model
    model = GaussianMLP(config.dims, config.dims)
    optim = torch.optim.Adam(params=model.parameters(), lr=config.learning_rate)
    permutation = torch.randperm(X.size()[0])
    Xi = X[permutation]
    yi = y[permutation]
...
def plot_ensemble_size_gaussian_model(gt_X, gt_y, X, y, config):
    ...
    ensemble_model = EnsembleGaussianMLP(config.dims, config.dims, ensemble_size=config.ensemble_size)
...
if __name__ == '__main__':
    ...
    # Ground truth data
    gt_X = torch.linspace(-6, 6, config.gt_samples * config.dims).view(config.gt_samples, config.dims)
    gt_y = gt_X**3
