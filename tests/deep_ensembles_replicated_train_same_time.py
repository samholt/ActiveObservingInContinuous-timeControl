...
class EnsembleGaussianMLP(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_units=100, ensemble_size=5):
        ...
        self.output_dim = output_dim
        self.models = []
        self.models = nn.ModuleList([GaussianMLP(input_dim, output_dim, hidden_units=hidden_units) for _ in range(ensemble_size)])

    def forward_per_ensemble(self, x):
        outputs = [self.models[j](x) for j in range(self.ensemble_size)]
        mean_for_each_network = torch.stack([o[0] for o in outputs])
        variance_for_each_network = torch.stack([o[1] for o in outputs])
        return mean_for_each_network, variance_for_each_network

    def forward(self, x):
        mean_for_each_network, variance_for_each_network = self.forward_per_ensemble(x)
...
def gaussian_NLL_multi(y, means, variances):
    return (torch.log(variances) + (torch.square(y - means) / variances)).mean(dim=-1).mean(dim=-1)
...
def plot_ensemble_size_gaussian_model(gt_X, gt_y, X, y, config):
    ...
    model = EnsembleGaussianMLP(1,1, ensemble_size=config.ensemble_size)
    permutation = torch.randperm(X.size()[0])
    ...
    for epoch in range(config.epochs):
        ...
        optim.zero_grad()
        means, variances = model.forward_per_ensemble(Xi)
        losses = gaussian_NLL_multi(yi, means, variances)
        [loss.backward(retain_graph=True) for loss in losses]
        # total_loss = loss
        # if epoch == 0:
        #     print(f'Network initial loss: {total_loss.item()}')
        # total_loss.backward()
        optim.step()
    # print(f'Network final loss: {total_loss.item()}')
