...
def cartpole_dynamics_dt(
    ...
    assert (
        state.shape[0] == perturbed_action.shape[0] == ts.shape[0]
    ), f"Failed on: {state.shape[0]} == {perturbed_action.shape[0]} == {ts.shape[0]}"
    if len(ts.shape) >= 2:
        assert ts.shape[1] == 1
    ts = ts.view(-1, 1)
    if state.shape[-1] == 5:
...
def pendulum_dynamics_dt(
    state, perturbed_action, ts, ACTION_LOW=-2.0, ACTION_HIGH=2.0, nu=1, friction=False, return_var=False
):
    assert state.shape[0] == perturbed_action.shape[0]
    ...
    if state.shape[-1] == 2:
        newthdot = thdot + (-3 * g / (2 * l) * torch.sin(th + torch.pi) + 3.0 / (m * l**2) * u) * ts
        newth = th + thdot * ts
        ...
        newthdot = thdot + (-3 * g / (2 * l) * torch.sin(th + np.pi) + 3.0 / (m * l**2) * u) * ts
        state = torch.cat((new_costheta, new_sintheta, newthdot), dim=1)
        return state


def acrobot_dynamics_dt(
    state, perturbed_action, ts, ACTION_LOW=-5.0, ACTION_HIGH=5.0, nu=2, friction=False, return_var=False
):
    assert state.shape[0] == perturbed_action.shape[0]
    assert perturbed_action.shape[1] == 2
    assert (
