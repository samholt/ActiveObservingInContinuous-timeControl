...
from overlay import create_env, setup_logger, start_virtual_display, plot_telem, step_env
import time
import torch
from functools import partial
import numpy as np
...
def mppi_with_model_evaluate_single_step_active_observing(model_name,
                                         env_name,
                                         sampling_policy,
                                         roll_outs=1000,
                                         time_steps=30,
    ...
    SAMPLING_POLICIES = ['discrete_planning', 'discrete_monitoring', 'continuous_planning', 'active_observing_control', 'random']
    ...
    if not (model_name == 'oracle' or model_name == 'random'):
        ...
        def dynamics(state, perturbed_action, ts_pred, encode_obs_time=encode_obs_time, model_name=model_name, return_var=True):
            if encode_obs_time and model_name == 'nl':
                perturbed_action = torch.cat((perturbed_action, torch.ones(1, device=device)).view(1,1,1).repeat(perturbed_action.shape[0],1,1))
        ...
        if config.oracle_var_type == 'fixed_oracle_var':
            ...
            def dynamics(*args, **kwargs):
                if not kwargs['return_var']:
                    ...
                    state_samples = state.view(K, 1, nx).repeat(1, oracle_var_monte_carlo_samples,1) + torch.normal(0,oracle_sigma, size=(K, oracle_var_monte_carlo_samples, nx)).to(device)
                    state_estimates = dynamics_oracle(state_samples.view(-1, nx), perturbed_action.repeat_interleave(oracle_var_monte_carlo_samples, dim=0), ts_pred.repeat(1,oracle_var_monte_carlo_samples).view(-1,1), **kwargs)
            ...
            def dynamics(*args, **kwargs):
                if not kwargs['return_var']:
                    ...
                    state, perturbed_action, ts_pred = args
                    K, nu = perturbed_action.shape[0], perturbed_action.shape[1]
                    perturbed_action_samples = perturbed_action.view(K, 1, nu).repeat(1,oracle_var_monte_carlo_samples,1) + torch.normal(0,oracle_sigma, size=(K, oracle_var_monte_carlo_samples, nu)).to(device)
                    state_estimates = dynamics_oracle(state.repeat_interleave(oracle_var_monte_carlo_samples, dim=0), perturbed_action_samples.view(-1, nu), ts_pred.repeat(1, oracle_var_monte_carlo_samples).view(-1,1), **kwargs)
    ...
    def loop():
        ...
        while it < iter_:
            ...
            if not config.multi_process_results:
                if print_out_costs_var:
                    logger.info(f"[{env_name}\t{model_name}\t|time_steps={time_steps}__dt_sim={config.dt_simulation}] action taken: {action.detach().cpu().numpy()} cost received: {-reward} | state {state.flatten()} time taken: {elapsed}s | {int(it/iter_*100)}% Complete \t | iter={it} \t| observed_times_diff={np.diff(np.array(observed_times))} \t| costs_var_sum={np.array(costs_std_median)}")
                else:
                    logger.info(f"[{env_name}\t{model_name}\t|time_steps={time_steps}__dt_sim={config.dt_simulation}] action taken: {action.detach().cpu().numpy()} cost received: {-reward} | state {state.flatten()} time taken: {elapsed}s | {int(it/iter_*100)}% Complete \t | iter={it} \t| observed_times_diff={np.diff(np.array(observed_times))}")
            if save_video:
        ...
        if config.plot_telem:
            ...
            ddict.update({'s': s, 'a': a, 'r': r, 'cost_std_plot': cost_std_plot, 'ri': ri, 'plot_seed': plot_seed, 'costs_std_stats': df.to_json().replace('{','<').replace('}','>')})
...
def seed_wrapper_mppi_with_model_evaluate_single_step_active_observing(run_args,
                                         roll_outs=1000,
                                         time_steps=30,
                                         lambda_=1.0,
                                         sigma=1.0,
    ...
    result = mppi_with_model_evaluate_single_step_active_observing(model_name=model_name,
                                                env_name=env_name,
                                                threshold=threshold,
                                                fixed_continuous_planning_observations=fixed_continuous_planning_observations,
                                                sampling_policy=sampling_policy,
...
if __name__ == '__main__':
    ...
    for env_name in [
        ...
        for sampling_policy, model_name in [
            ...
            for seed in range(config.seed_start, config.seed_runs + config.seed_start):
                args_for_runs.append((env_name, model_name, sampling_policy, seed, threshold, fixed_continuous_planning_observations))
    multi_seed_wrapper_mppi_with_model_evaluate_single_step = partial(seed_wrapper_mppi_with_model_evaluate_single_step_active_observing,
    ...
    if debug_main:
        for args_for_run in args_for_runs:
                ...
                logger.info(f"[Policy short form][{printable_result['env_name']}|{printable_result['sampling_policy']}|{printable_result['model_name']}|{printable_result['dt_plan']}] total_reward:{printable_result['total_reward']}\t| state_reward:{printable_result['state_reward']}\t| observations_taken:{printable_result['observations_taken']}\t| observing_var_threshold:{printable_result['observing_var_threshold']}\t|")
                if config.plot_telem:
                    logger.info(f"[Policy telem path] {result['telem_file_path']}")
                results.append(result)
    else:
        for i, result in tqdm(enumerate(pool_outer.imap_unordered(multi_seed_wrapper_mppi_with_model_evaluate_single_step, args_for_runs)), total=len(args_for_runs), smoothing=0):
                printable_result = {k : v.tolist() if isinstance(v, np.ndarray) else v for k,v in result.items()}
                logger.info(f'[Policy evaluation complete] {printable_result}')
                logger.info(f"[Policy short form][{printable_result['env_name']}|{printable_result['sampling_policy']}|{printable_result['model_name']}|{printable_result['dt_plan']}] total_reward:{printable_result['total_reward']}\t| state_reward:{printable_result['state_reward']}\t| observations_taken:{printable_result['observations_taken']}\t| observing_var_threshold:{printable_result['observing_var_threshold']}\t|")
                if config.plot_telem:
    ...
    from process_results.plot_util import normalize_means, generate_main_results_table
