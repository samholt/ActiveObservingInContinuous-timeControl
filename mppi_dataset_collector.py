...
def inner_mppi_with_model_collect_data(seed,
    ...
    def loop():
        ...
        while it < iter_:
            ...
            total_reward += reward
            if log_debug:
                logger.info(f"action taken: {action.detach().cpu().numpy()} cost received: {-reward} | state {state.flatten()} ts {tsn.detach().cpu().numpy()} | time taken: {elapsed}s | {int(it/iter_*100)}% Complete \t | iter={it}")
            if save_video:
...
def mppi_with_model_collect_data(model_name,
                                env_name,
                                roll_outs=1000,
                                time_steps=30,
                                lambda_=1.0,
...
if __name__ == '__main__':
    ...
    for env_name in ['oderl-cartpole', 'oderl-acrobot', 'oderl-pendulum']:
        ...
        results = mppi_with_model_collect_data(model_name='oracle',
                                                env_name=env_name,
