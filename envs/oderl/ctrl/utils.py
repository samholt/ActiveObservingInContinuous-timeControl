...
def plot_model(ctrl, D, rep_buf=10, H=None, L=10, fname=None, verbose=False, savefig=True):
    with torch.no_grad():
        ...
        st_hat, rt_hat, at_hat, t = ctrl.forward_simulate(tobs, st[:, 0, :], g, L=L, compute_rew=False)
        if verbose:
            print(st_hat.shape)
...
def plot_test(ctrl, D, N=None, H=None, L=10, fname=None, verbose=False, savefig=True):
    with torch.no_grad():
        ...
        st_hat, rt_hat, at_hat, t = ctrl.forward_simulate(H, st[:, 0, :], g, L=L, compute_rew=False)
        if verbose:
            print(st_hat.shape)
        plot_sequences(ctrl, fname, tobs, st, at, rt, t, st_hat, at_hat, rt_hat, verbose=verbose, savefig=savefig)
...
def train_loop(ctrl, D, fname, Nround, **kwargs):
    ...
    for round in range(r0, r0 + Nround):
        ...
        print(
            "Model tested. True/imagined reward sum is {:.2f}/{:.2f}".format(true_test_rewards, imagined_test_rewards)
        )
        print(f"Minimum test reward is {test_rewards[...,Tup:].min().item()}")
        if (test_rewards[..., Tup:] < 0.8).sum() == 0:
...
def train_policy(
    ...
    for itr in range(Niter):
        ...
        print_log = "Iter:{:4d}/{:<4d},  opt. target:{:.3f}  mean reward:{:.3f}  ".format(
            itr, Niter, np.mean(opt_objs), np.mean(rewards)
        ...
        print_log += "first/final value error:{:.3f}/{:.3f}  kl:{:.3f}".format(
            first_val_err, td_error.item(), ctrl.V.kl().item()
        )
...
def train_dynamics(
    ...
    for k in range(Niter):
        ...
        if verbose and k % (Niter // print_times) == 0:
            print(
                "Iter:{:4d}/{:<4d} loss:{:<.3f} lhood:{:<.3f} KL:{:<.3f} mse:{:<.3f}  grad norm:{:.4f} T:{:d}".format(
                    k,
                    Niter,
                    np.mean(losses),
                    np.mean(lhoods),
...
def collect_test_sequences(ctrl, D, N=1, reset=True, explore=False, sf=0.1):
    with torch.no_grad():
        ...
        T = D.T
        if reset:
            s0 = torch.stack([torch.tensor(env.reset(), dtype=torch.float32) for _ in range(N)]).to(ctrl.device)
        else:
...
def gradient_match(ctrl, D, Niter=5000, eta=5e-3, verbose=False, L=10, print_times=100, rep_buf=-1, opt="adam"):
    rep_buf = D.N if rep_buf < 1 else rep_buf
    ds = (D.s[-rep_buf:, 1:] - D.s[-rep_buf:, :-1]).reshape(-1, ctrl.env.n)
    dt = (D.ts[-rep_buf:, 1:] - D.ts[-rep_buf:, :-1]).reshape(-1, 1)
    ds_dt_opt = ds / dt  # _,n
