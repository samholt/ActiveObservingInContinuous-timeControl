...
class CTRL(nn.Module):
    def __init__(self, env, dynamics, **kwargs):
        ...
        self.make_dynamics_model(
            nl_f=kwargs["nl_f"], nn_f=kwargs["nn_f"], act_f=kwargs["act_f"], dropout_f=kwargs["dropout_f"]
        )
        self.V = BNN(
            self.env.n, 1, n_hid_layers=kwargs["nl_V"], act=kwargs["act_V"], n_hidden=kwargs["nn_V"], bnn=False
        )
    ...
    def forward_simulate(self, H_ts, s0, g, f=None, L=10, tau=None, compute_rew=False):
        ...
        if isinstance(H_ts, float) or isinstance(H_ts, int):
            return self._f.forward_simulate(
                solver=self.solver, H=H_ts, s0=s0, f=f, g=g, L=L, tau=tau, compute_rew=compute_rew
            )
        else:
            return self._f.forward_simulate_nonuniform_ts(
                solver=self.solver, ts=H_ts, s0=s0, f=f, g=g, L=L, tau=tau, compute_rew=compute_rew
            )
