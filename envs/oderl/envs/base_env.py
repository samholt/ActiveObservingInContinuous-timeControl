...
class BaseEnv(gym.Env, metaclass=ABCMeta):
            ...
            if self.ts_grid == "fixed":
                ...
                ts = torch.cat(
                    (
                        ...
                        torch.distributions.exponential.Exponential(1 / self.dt).sample([1]).to(device),
                    )
                )
            else:
    ...
    def integrate_system(self, T, g, s0=None, N=1, return_states=False):
        ...
        with torch.no_grad():
            ...
            st = odeint(
                odefnc, s0, ts, rtol=self.solver["rtol"], atol=self.solver["atol"], method=self.solver["method"]
            )
    ...
    def batch_integrate_system_double_time(self, is0s, actions, device=None):
        ...
        with torch.no_grad():
            ...
            for a in actions:
                ...
                st = odeint(
                    odefnc, s0s, ts, rtol=self.solver["rtol"], atol=self.solver["atol"], method=self.solver["method"]
                )
                sb_l.append(st[-2, :, :])
                sn_l.append(st[-1, :, :])
            sn = torch.stack(sn_l)
