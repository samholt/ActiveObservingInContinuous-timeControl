...
class BaseEnv(gym.Env, metaclass=ABCMeta):
    ...
    def __init__(self, dt, n, m, act_rng, obs_trans, name, state_actions_names, \
                 device, solver, obs_noise, ts_grid, ac_rew_const=0.01, vel_rew_const=0.01, n_steps=200):
        self.dt = dt
        self.n = n
        self.m = m
    ...
    def build_time_grid(self,T=None, only_one_step=True, device=None):
        ...
        if only_one_step:
            if self.ts_grid=='fixed':
                ...
                ts = torch.cat((torch.tensor([0.0], device=device), (torch.rand(1,device=device)*2*self.dt)))
            elif self.ts_grid=='exp':
                ts = torch.cat((torch.tensor([0.0], device=device), torch.distributions.exponential.Exponential(1/self.dt).sample([1]).to(device)))
            else:
                raise ValueError('Time grid parameter is wrong!')
            return ts
        else:
            if self.ts_grid=='fixed':
                ts = torch.arange(T,device=device) * self.dt  
            elif self.ts_grid=='uniform' or self.ts_grid=='random':
                ts = (torch.rand(T,device=device)*2*self.dt).cumsum(0)
            elif self.ts_grid=='exp':
    ...
    def batch_integrate_system_double_time(self, is0s, actions, device=None):
        ...
        with torch.no_grad():
            ...
            for a in actions:
                ...
                def odefnc(t, s):
                    return self.torch_rhs(s, ab)
                st = odeint(odefnc, s0s, ts, rtol=self.solver['rtol'], atol=self.solver['atol'], \
                        method=self.solver['method'])
                sb_l.append(st[-2,:,:])
                sn_l.append(st[-1,:,:])
            sn = torch.stack(sn_l)
