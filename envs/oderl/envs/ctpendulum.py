...
class CTPendulum(BaseEnv):
    ...
    def __init__(
        self, dt=0.1, device="cpu", obs_trans=True, obs_noise=0.0, ts_grid="fixed", solver="dopri8", friction=False
    ):
        name = "pendulum"
        if obs_trans:
        ...
        super().__init__(
            dt, 2 + obs_trans, 1, 2.0, obs_trans, name, state_action_names, device, solver, obs_noise, ts_grid
        )
        self.N0 = 3
        self.Nexpseq = 0
        self.g = 10.0
    ...
    def reset(self):
        ...
        rand_state = self.np_random.uniform(low=-0.1, high=0.1, size=(2,))
        rand_state[0] += np.pi
    ...
    def torch_rhs(self, state, action):
        ...
        if state.shape[-1] == 2:
            ...
            return torch.stack(
                [thdot, (-3 * g / (2 * l) * torch.sin(th + np.pi) + 3.0 / (m * l**2) * action[..., 0])], -1
            )
            ...
            return torch.stack(
                [
                    -sinth * thdot,
                    costh * thdot,
                    (-3 * g / (2 * l) * torch.sin(th + np.pi) + 3.0 / (m * l**2) * action[..., 0]),
                ],
                -1,
            )
    ...
    def diff_obs_reward_(self, state, exp_reward=False):
        ...
        if exp_reward:
            ...
            return state_reward + self.vel_rew_const * velocity_reward  # works superb
        # return (state_reward + self.vel_rew_const*velocity_reward).exp() # works superb
