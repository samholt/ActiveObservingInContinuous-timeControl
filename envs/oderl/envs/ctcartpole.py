...
class CTCartpole(BaseEnv):
    ...
    def __init__(
        ...
        x_threshold = 2.4
        super().__init__(
            dt, 4 + obs_trans, 1, 3.0, obs_trans, name, state_action_names, device, solver, obs_noise, ts_grid
        )
        ...
        self.observation_space = spaces.Box(
            low=-np.array([5.0, 20.0, 1.0, 1.0, 30.0]), high=-np.array([5.0, 20.0, 1.0, 1.0, 30.0]), dtype=np.float64
        )
    ...
    def df_du(self, state):
        if state.shape[-1] == 4:
            ...
            x, _, costheta, sintheta, _ = state[..., 0], state[..., 1], state[..., 2], state[..., 3], state[..., 4]
    ...
    def torch_rhs(self, state, action):
        ...
        if self.friction:
            temp = (
                force + self.polemass_length * theta_dot * theta_dot * sintheta - self.friction_cart * torch.sign(x_dot)
            ) / self.total_mass
            thetaacc = (
                self.gravity * sintheta - costheta * temp - self.friction_pole * theta_dot / self.polemass_length
    ...
    def diff_obs_reward_reduced_state(
        self,
        s,
        penalize_vel=False,
        exp_reward=False,
        ...
        if self.swing_up:
            ...
            if state_constraint:
                ...
                position_error = err[:, 0] ** 2 + torch.exp(
                    err[:, 0] * 10.0 + 7.0
                )  # Doesn't violate the state constraint
                angle_error = err[:, 1] ** 2
            ...
            return (
                (cos_th > np.cos(self.theta_threshold_radians)) * (x < self.x_threshold) * (x > -self.x_threshold)
            ).float()

    def diff_obs_reward_(
        self,
        s,
        penalize_vel=False,
        exp_reward=False,
        ...
        if s.shape[-1] == 4:
            ...
            x, xdot, cos_th_len, sin_th_len, thetadot = s[..., :1], s[..., 1:2], s[..., 2:3], s[..., 3:4], s[..., 4:]
            cos_th, sin_th = cos_th_len / self.length, sin_th_len / self.length
        if self.swing_up:
            ee_pos = torch.cat([x + sin_th_len, cos_th_len], -1)
            if change_goal:
    ...
    def render(self, mode="human", *args, **kwargs):
        ...
        if self.viewer is None:
            ...
            axle.v = [
                (screen_width / 2 - 5, polelen + carty - 5),
                (screen_width / 2 - 5, polelen + carty + 5),
                (screen_width / 2 + 5, polelen + carty + 5),
                (screen_width / 2 + 5, polelen + carty - 5),
            ]
