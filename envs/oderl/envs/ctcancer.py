...
class CTCancer(BaseEnv):
    ...
    def __init__(self, dt=0.1, device='cpu', obs_trans=True, obs_noise=0.0, ts_grid='fixed', solver='dopri8', friction=False, probabilistic=False):
        name = 'cancer'
        state_action_names = ['cancer_volume','chemo_action','radio_action']
        # self.reward_range = [-17.0, 0.0] # for visualization
        super().__init__(dt, 2, 2, 2.0, obs_trans, name, \
            state_action_names, device, solver, obs_noise, ts_grid, ac_rew_const=0.001) #0.001)
            # Params
        self.action_max = 2.0
    ...
    #################### override ##################
    def reset(self):
        state = self.np_random.uniform(low=calc_volume(13)*0.98, high=calc_volume(13)*0.99, size=(2,)).astype('float32')
