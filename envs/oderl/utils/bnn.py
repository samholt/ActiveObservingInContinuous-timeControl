...
class BNN(nn.Module):
        ...
        if bnn:
            ...
            for i, (n_in, n_out) in enumerate(zip(layers_dim[:-1], layers_dim[1:])):
                ...
                self.bias_logsigs.append(
                    None if not bias else Parameter(torch.Tensor(1, n_out), requires_grad=requires_grad)
                )
    ...
    def __sample_weights(self, L, noise_vec=None):
        if self.bnn:
            ...
            for weight_mu, weight_sig in zip(self.weight_mus, self.weight_logsigs):
                p = weight_mu.numel()
                weights.append(
                    weight_mu + noise_vec[:, i : i + p].view(L, *weight_mu.shape) * self.__transform_sig(weight_sig)
                )
                i += p
            if self.bias:
                biases = []
                for bias_mu, bias_sig in zip(self.bias_mus, self.bias_logsigs):
                    p = bias_mu.numel()
                    biases.append(
                        bias_mu + noise_vec[:, i : i + p].view(L, *bias_mu.shape) * self.__transform_sig(bias_sig)
                    )
                    i += p
            else:
                biases = [
                    torch.zeros([L, 1, weight_mu.shape[1]], device=weight_mu.device) * 1.0
                    for weight_mu, bias_mu in zip(self.weight_mus, self.bias_mus)
                ]  # list of zeros
        else:
    ...
    def __repr__(self):
        ...
        for i, (weight, act) in enumerate(zip(self.weight_mus, self.acts)):
            str_ += "Layer-{:d}: ".format(i + 1) + "".join(str([*weight.shape][::-1])) + "\t" + str(act) + "\n"
        return str_
