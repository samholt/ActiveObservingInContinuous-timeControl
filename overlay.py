...
def create_oderl_env(env_name, dt=0.05, ts_grid="fixed", noise=0.0, friction=False, device=device):
    ...
    env = ENV_CLS(
        dt=dt, obs_trans=True, device=device, obs_noise=noise, ts_grid=ts_grid, solver="euler", friction=friction
    )
    return env
...
def compute_state_actions(rand, samples_per_dim, device_h, state_max, action_max, state_min, action_min):
    ...
    if rand:
        s0s_dist = (
            torch.rand(
                samples_per_dim ** state_max.shape[0], state_max.shape[0], dtype=TORCH_PRECISION, device=device_h
            )
            - 0.5
        ) * 2.0  # [-1,1]
        ...
        actions_dist = (
            torch.rand(samples_per_dim, action_max.shape[0], dtype=TORCH_PRECISION, device=device_h) - 0.5
        ) * 2.0
        ...
        if state_max.shape[0] == 4:
            x, y, z, k = torch.meshgrid(
                torch.linspace(state_min[0], state_max[0], samples_per_dim, device=device_h),
        ...
        if action_max.shape[0] == 1:
            ...
            a1, a2 = torch.meshgrid(
                torch.linspace(action_min[0], action_max[0], samples_per_dim, device=device_h),
...
def generate_irregular_data_time_multi(
    train_env_task,
    env,
    ...
    if train_env_task == "oderl-cartpole":
        state_max = torch.tensor(
            [5.0, 20, torch.pi, 30]
        )  # state_max = torch.tensor([7.0, 20, torch.pi, 30]) # state_max = torch.tensor([1.0, 1.0, torch.pi/8.0, 3.0])
    ...
    if reuse_state_actions_when_sampling_times:
        s0s, actions = compute_state_actions(
            rand, samples_per_dim, device_h, state_max, action_max, state_min, action_min
        )
...
def load_expert_irregular_data_time_multi(train_env_task, encode_obs_time=True, config={}):
