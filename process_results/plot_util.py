...
def normalize_means(df):
    ...
    for env_name in df_means.env_name.unique():
        ...
        df.loc[df.env_name == env_name, "total_reward"] = (
            (df[df.env_name == env_name].total_reward - random_row.total_reward)
            / (best_row.total_reward - random_row.total_reward)
        ) * 100.0
        df.loc[df.env_name == env_name, "state_reward"] = (
            (df[df.env_name == env_name].state_reward - random_row.state_reward)
            / (best_row.state_reward - random_row.state_reward)
        ) * 100.0
    return df
...
def normalize_means_cp_matching_obs(df_in):
    ...
    for env_name in df_means.env_name.unique():
        ...
        df_in.loc[df_in.env_name == env_name, "total_reward"] = (
            (df_in[df_in.env_name == env_name].total_reward - random_row.total_reward)
            / (best_row.total_reward - random_row.total_reward)
        ) * 100.0
        df_in.loc[df_in.env_name == env_name, "state_reward"] = (
            (df_in[df_in.env_name == env_name].state_reward - random_row.state_reward)
            / (best_row.state_reward - random_row.state_reward)
        ) * 100.0
    return df_in
...
def generate_main_results_table_cp_with_unique_obs(df_results, wandb=None):
    ...
    sf = 3
    env_name_map = {
        "oderl-cartpole": "Cartpole",
        "oderl-pendulum": "Pendulum",
        "oderl-acrobot": "Acrobot",
        "oderl-cancer": "Cancer",
    }

    env_name_ordering = {"oderl-cartpole": 2, "oderl-pendulum": 3, "oderl-acrobot": 1, "oderl-cancer": 0}

    sampling_policy_map = {
        "discrete_monitoring": "Discrete Monitoring",
    ...
    table_lines.append(
        "".join([r"&  \multicolumn{3}{c|}{" + env_name_map[env_name] + "}" for env_name in df_out.env_name.unique()])
        + r"\\"
    )
    table_lines.append(
        r"Policy " + r"& $\mathcal{U}$ & $\mathcal{R}$ & $\mathcal{O}$" * df_out.env_name.nunique() + r"\\"
    )
    ...
    for sampling_policy in df_out.sampling_policy.unique():
        ...
        for env_name in df_out.env_name.unique():
            ...
            line += (
                r"&"
                + f"{row.total_reward['mean'].iloc[0]:.{sf}g}"
                + r"$\pm$"
                + f"{row.total_reward['std'].iloc[0]:.{sf}g}"
                + r"&"
                + f"{row.state_reward['mean'].iloc[0]:.{sf}g}"
                + r"$\pm$"
                + f"{row.state_reward['std'].iloc[0]:.{sf}g}"
                + r"&"
                + f"{row.observations_taken['mean'].iloc[0]:.{sf}g}"
                + r"$\pm$"
                + f"{row.observations_taken['std'].iloc[0]:.{sf}g}"
            )
        line += r"\\"
        table_lines.append(line)
...
def generate_main_results_table(df_results, wandb=None, use_95_ci=True):
    ...
    for sampling_policy in df_out.sampling_policy.unique():
        ...
        for env_name in df_out.env_name.unique():
            ...
            if sampling_policy == "active_observing_control":
                line += (
                    (r"&\bf" if row.total_reward["mean"].iloc[0] > 100.0 else r"&")
                    + f"{row.total_reward['mean'].iloc[0]:.{sf}g}"
                    + r"$\pm$"
                    + f"{row.total_reward[error_metric].iloc[0]:.{sf}g}"
                    + (r"&\bf" if row.state_reward["mean"].iloc[0] > 100.0 else r"&")
                    + f"{row.state_reward['mean'].iloc[0]:.{sf}g}"
                    + r"$\pm$"
                    + f"{row.state_reward[error_metric].iloc[0]:.{sf}g}"
                    + r"&"
                    + f"{row.observations_taken['mean'].iloc[0]:.{sf}g}"
                    + r"$\pm$"
                    + f"{row.observations_taken[error_metric].iloc[0]:.{sf}g}"
                )
            else:
                line += (
                    r"&"
...
def gen_match_cp_obs_table(df, use_95_ci=True):
    ...
    if use_95_ci:
        df_out = (
            aoc_with_normalization_reference_baselines.groupby(
                ["env_name", "sampling_policy", "model_name", "observing_var_threshold"]
            )
            .agg([np.mean, ci])
            .reset_index()
        )
        cp_dif_obs = (
            cp_with_different_obs.groupby(
                ["env_name", "sampling_policy", "model_name", "fixed_continuous_planning_observations"]
            )
            .agg([np.mean, ci])
            .reset_index()
        )
        ...
        for env_name in df_out.env_name.unique():
            ...
            if use_95_ci:
                line += (
                    ...
                    + f"{row.total_reward['mean'].iloc[0]:.{sf}g}"
                    + r"$\pm$"
                    + f"{row.total_reward['ci'].iloc[0]:.{sf}g}"
                    + r"&"
                    + f"{row.state_reward['mean'].iloc[0]:.{sf}g}"
                    + r"$\pm$"
                    + f"{row.state_reward['ci'].iloc[0]:.{sf}g}"
                    + r"&"
                    + f"{row.observations_taken['mean'].iloc[0]:.{sf}g}"
                    + r"$\pm$"
                    + f"{row.observations_taken['ci'].iloc[0]:.{sf}g}"
                )
            else:
                line += (
                    r"&"
    ...
    for i, row in cp_dif_obs.iterrows():
        ...
        for env_name in cp_dif_obs.env_name.unique():
            if use_95_ci:
                line += (
                    r"&"
                    + f"{row.total_reward['mean']:.{sf}g}"
                    + r"$\pm$"
                    + f"{row.total_reward['ci']:.{sf}g}"
                    + r"&"
                    + f"{row.state_reward['mean']:.{sf}g}"
                    + r"$\pm$"
                    + f"{row.state_reward['ci']:.{sf}g}"
                    + r"&"
                    + f"{row.observations_taken['mean']:.{sf}g}"
                    + r"$\pm$"
                    + f"{row.observations_taken['ci']:.{sf}g}"
                )
            else:
                line += (
                    r"&"
                    + f"{row.total_reward['mean']:.{sf}g}"
                    + r"$\pm$"
                    + f"{row.total_reward['std']:.{sf}g}"
                    + r"&"
                    + f"{row.state_reward['mean']:.{sf}g}"
                    + r"$\pm$"
                    + f"{row.state_reward['std']:.{sf}g}"
                    + r"&"
                    + f"{row.observations_taken['mean']:.{sf}g}"
                    + r"$\pm$"
                    + f"{row.observations_taken['std']:.{sf}g}"
                )
        line += r"\\"
        table_lines.append(line)
    table_lines.append(r"\bottomrule")
